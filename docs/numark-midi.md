...
| A pitch-bend − / + | Note 65 / 66 | Note 56 (`0x38`) / 57 (`0x39`) | Momentary press/activation feedback |
| B pitch-bend − / + | Note 69 / 70 | Note 72 (`0x48`) / 73 (`0x49`) | Momentary press/activation feedback |
...
The documented pitch-bend LEDs are momentary rather than part of the persistent state snapshot. A physical Note On lights the matching hardware LED and on-screen −/+ button until its matching release; held duplicate Note On messages remain suppressed. A GUI mouse, touch, or keyboard activation performs the same single ±0.1 action and lights the on-screen icon and, when the matching MIDI output is connected, the hardware LED for a bounded 180 ms pulse after the last repeated activation. Blur/cancel clears the GUI pulse, while input detach, explicit disconnect, and adapter/component teardown clear all remaining sources and timers. The physical pitch-slider center LEDs (52/67) remain off because the pitch sliders are intentionally unmapped; the app's pitch state is controlled by the repurposed Gain knobs, so lighting the sliders would fabricate an association. Unassigned surface controls remain off. The mapped FX knobs, Fine Pitch/sample selectors, Gain/pitch knobs, jog wheels, volume faders, crossfader, Browse encoder, and Browse press have no separate LED in the wiki table; none is invented.
...
The shared Help/MIDI corner owns the connection controls; Mixer owns the single adapter. Main input and lighting/output status or error text is rendered once below the Music Crate, outside the deck/crossfader region.
...
const midi = createTotalControlMidi({
  ...
  onPitchStepFeedback(event) { /* mirror momentary pitch −/+ feedback in the GUI */ },
...
// GUI activation: exact hardware/UI feedback pulse; no audio action occurs here.
midi.pulsePitchStep('left', -0.1);
// Blur or pointer cancellation can end only the GUI-owned source early.
midi.clearPitchStep('left', -0.1);
...
- `node --test tests/*.test.mjs` uses explicitly synthetic messages and injected mock Web MIDI ports. It checks all intended persistent state-to-note mappings, the four momentary pitch output notes and exact `0x90` on/off bytes, physical held/release behavior, the bounded GUI pulse and overlapping input sources, safe output selection, missing/failed output isolation, reconnect sync, explicit disconnect, and teardown while output open is pending. It also executes the latched Sync activity lifecycle and shipped UI/layout contracts.
...
- The approved headed-browser lane remains the only allowed visual acceptance path; source, transform, and synthetic tests do not prove physical controller LEDs, phone hit-testing, audio playback, or rendered geometry.
