...
const Deck = ({ 
  ...
  // State to track beat for visual effects
  const [isBeat, setIsBeat] = React.useState(false);


  // Beat detection callback
  const handleBeatDetected = React.useCallback((beat) => {
    setIsBeat(beat);
    setTimeout(() => setIsBeat(false), 80);
  }, []);

  // Add a ref to store the beat detector reset function
  const beatDetectorResetRef = React.useRef(null);

  // Add a function to receive the reset function from BeatDetector
  const handleBeatDetectorRef = React.useCallback((resetFn) => {
    beatDetectorResetRef.current = resetFn;
  }, []);
  ...
  React.useEffect(() => {
    if (seekVersion > 0 && beatDetectorResetRef.current) {
      beatDetectorResetRef.current();
    }
  }, [seekVersion]);
    ...
    <div className="card bg-dark">
      ...
      <div className="card-body">
        ...
        <BeatDetector
          ...
          onResetRef={handleBeatDetectorRef}
          renderDeckControls={(sensitivityControl, beatStatus) => (
            <Fragment>
